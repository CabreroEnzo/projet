...
int main() {
    printf("[DEBUG] Avant : 'a', Apr̬s : '%c' ", toupper('a'));
    int tailleDuMot;
    ...
    do{
        ...
        do {
            printf("Saisissez votre mot (carract̬re par carract̬re) \n");
        ...
        if(estTrouve == 1){
            printf("Bravo ! Vous avez trouv̩ le mot ");
            for (int i = 0; i < tailleDuMot; ++i) {
                printf("%c",motEnCours[i]);
            }
            printf(" en %d coup !",7-nbCoupRestant+1);
        }else{
            printf("Dommage... le mot a trouver ̩tait ");
            for (int i = 0; i < tailleDuMot; ++i) {
                printf("%c",motATrouver[i]);
            }
        }
        getchar();
        printf("\nVoulez-vous relanc̩ une partie ? [O,N]: ");
